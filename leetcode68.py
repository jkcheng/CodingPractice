...
class mySolution:
    def fullJustify(self, words: List[str], maxWidth: int) -> List[str]:
        # output array
        lines = []
        # track current line width and current line
        width = 0
        line = []

        # process each line at a time by adding words until maxWidth reached
        for word in words:
            ...
            # special case for processing one-word line, append blank spaces to right
            if len(line) == 1:
                fullline = ' '.join(line)
                fullline = line.append(" " * (maxWidth - len(line)))
            # calculate number of spaces left to pad line and add between words
            else:
                space, extra = divmod(
                    maxWidth - width,
                    len(line) - 1
